...
static void _FillRect(GUI_DEVICE * pDevice, int x0, int y0, int x1, int y1) {
    ...
    if (GUI_pContext->DrawMode & LCD_DRAWMODE_XOR) 
	{
		...
		vLcdDrawRect(x0, y0, x1 - x0 + 1, y1 - y0 + 1, PixelIndex);
    }
}
...
static void _DrawBitmap(GUI_DEVICE * pDevice, int x0, int y0,
  ...
  case 16:
    for (i = 0; i < ySize; i++) {
		...
		vLcdDrawRectColor(x0, y0, xSize, ySize, (const uint16_t *) pData);
    }
    break;
  //
  // Only required for 32bpp color depth of target. Should be removed otherwise.
