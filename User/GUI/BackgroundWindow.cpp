...
void BackgroundWindow::onCallback(WM_MESSAGE *pMsg)
{
	switch (pMsg->MsgId)
	{
		...
		case WM_TIMER:
			if (UiProgBar::instance().isShowing()) 
			{
				UiProgBar::instance().update();
...
BackgroundWindow::BackgroundWindow(void)
{
	...
	for (uint16_t i = 0; i < GUI_COUNTOF(mCountArray); i ++) { mCountArray[i] = 0; }
}

void BackgroundWindow::onProgBarTimeOut(void)
{
	UiProgBar::instance().dismiss();
}
...
void BackgroundWindow::deliver(void)
{
	...
	DeliverUart::instance().setAction(0x09)
		.append1(mCountArray[0])
		.append1(mCountArray[1])
		.append1(mCountArray[2])
		.append1(mCountArray[3])
		.append1(mCountArray[4]).buildAndWrite();
	clearAllCount();
	UiProgBar::instance().show(this, 180);
	hTimer = WM_CreateTimer(WM_HBKWIN, 0, 500, 0);
}

void BackgroundWindow::onKeyClick(uint16_t keyValue)
{
	if (keyValue >= ID_KEY_VALUE_1 && keyValue <= ID_KEY_VALUE_5) 
	{
		uint16_t v = keyValue - ID_KEY_VALUE_1;
		if (getAllCount() < 5) {
			mCountArray[ v ] ++;
		} else {
			clearAllCount();
	...
	switch (keyValue)
	{
		case ID_SHOW_PASSWORD_WINDOW:
			PasswordWindow::instance().show();
			break;
		...
		case ID_KEY_VALUE_ENT:
			...
			PayWindow::instance().show(WM_HBKWIN, getAllPrice());
			break;
		
		case ID_MSG_START_DELIVER:
			deliver();
			break;
		...
		case ID_MSG_DELIVER_ERROR:
			UiProgBar::instance().error();
			break;
		
		case ID_MSG_DELIVER_OK:
			UiProgBar::instance().success();
			break;
	}
}

bool BackgroundWindow::isShowing(void)
{
	return true;
}
